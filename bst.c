...
struct Node *_addNode(struct Node *cur, TYPE val)
{
    ...
    //value is less than left child
    else if (compare(val, cur->val) == -1) {
        cur->left = _addNode(cur->left, val);
    }
    //value is greater than right child
    else {
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    while (temp != 0) {
        ...
        if (compare(temp->val, val) == 0) { return 1; }
        else if (compare(temp->val, val) == 1) { temp = temp->left; }
...
{
    //assume value is in the tree?
    
    //base case (value has been found)
